	program integrais
		...
		open(20,file='tab2_out.dat')
			...
			do i=1,n
				...
				if(abs(r_t(k)-r).lt.abs(r_s(k)-r)) then	
				if(abs(r_t(k)-r).lt.abs(r_b(k)-r))then
		print*,n," A regra do Trapezio, por ter o menor erro"
					else
		print*,n," A regra de Boole, por ter o menor erro"
					endif
				else
					if(abs(r_s(k)-r).lt.abs(r_b(k)-r))then
		print*,n," A regra de Simpson, por ter o menor erro"
					else
		print*,n," A regra de Boole, por ter o menor erro"
					endif
				endif
